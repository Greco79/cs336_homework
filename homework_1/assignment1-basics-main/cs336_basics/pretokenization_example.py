...
### ====== Part 2: 初始化 vocab / pattern / token_freqs ====== ###
vocab = {b"<|endoftext|>": 0}
vocab_size = 1
target_vocab_size = 10000 # 暂时是10000
...
### ====== Part 3: pre 分词函数 ====== ###
def update_token_freqs(text, token_freqs, pattern):
...
with open("tests/fixtures/tinystories_sample.txt", "rb") as f:
    ...
    for start, end in zip(boundaries[:-1], boundaries[1:]):
        ...
        update_token_freqs(chunk, token_freqs, PAT)
    print(f"[Part 3] Pre-tokenization took {time.time() - t2:.2f}s")

## ===== Part 5 =====
t3 = time.time()
for _ in tqdm(range(num_merges), desc="Merging BPE pairs"):
    pair_freqs = get_pairs(token_freqs)
    if not pair_freqs:
        break
